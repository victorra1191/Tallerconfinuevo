    ...
    <div className="min-h-screen bg-gray-50 font-helvetica pb-10">
      ...
      <div className="bg-white shadow-md border-b sticky top-0 z-20 px-4 py-4 container mx-auto flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input placeholder="Buscar productos..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 h-12" />
        </div>
      ...
      <div className="container mx-auto px-4 py-10">
        ...
        {loading ? (
          <div className="flex flex-col items-center py-20">
            ...
            <p className="text-gray-500">Cargando inventario real de Neon...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredProducts.map((product) => (
              <Card key={product.id} className="overflow-hidden border-0 shadow-lg flex flex-col h-full bg-white group">
                <div className="relative h-64 p-6 flex items-center justify-center border-b bg-white">
                  ...
                  {/* USAMOS EL CAMPO 'image' REAL DE LA BASE DE DATOS */}
                  <img 
                ...
                <CardContent className="p-6 flex flex-col flex-1">
                  <h3 className="text-xl font-bold text-[#D71920] text-center mb-2 min-h-[3.5rem] line-clamp-2">
                    {product.name}
                  </h3>
                  {/* MAPEO REAL DEL CAMPO 'description' */}
                  <p className="text-sm text-gray-500 text-center mb-4 italic min-h-[2.5rem] line-clamp-2">
                    {product.description || "Producto automotriz para uso especializado."}
                  </p>
                  
                  <div className="space-y-3 mb-6 border-t pt-4">
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500 font-bold uppercase text-[10px]">SKU:</span>
                      <span className="font-mono font-bold text-gray-900">{product.sku}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500 font-bold uppercase text-[10px]">Tipo:</span>
                      <Badge variant="outline" className="text-[10px] font-bold">{product.type}</Badge>
                    </div>
                    <div className="text-center pt-2">
                      <span className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Usos recomendados:</span>
                      <p className="text-xs text-gray-600 font-medium">Motores de autos, motos o maquinaria</p>
                    </div>
                  </div>

                  <div className="mt-auto">
                    <div className="text-center mb-4">
                      <span className="text-3xl font-black text-[#D71920]">${Number(product.price).toFixed(2)}</span>
                    </div>
                    <div className="grid grid-cols-5 gap-2">
                      <Button 
                        ...
                        className="col-span-4 bg-[#D71920] hover:bg-[#b01319] font-bold h-12 text-white shadow-md"
                      >
                      ...
                      <Button 
                        ...
                        className="col-span-1 border-gray-200 h-12 hover:bg-gray-50 flex items-center justify-center"
                      >
