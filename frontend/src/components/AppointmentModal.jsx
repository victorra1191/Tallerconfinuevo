...
const AppointmentModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await mockFunctions.bookAppointment(appointmentData);
      
      if (result.success) {
        // Also try to create event in Zoho Calendar
        try {
          const zohoEventData = {
            title: `Servicio: ${formData.serviceType}`,
          ...
          // Send to Zoho Calendar endpoint
          const token = localStorage.getItem('admin_token');
          if (token) {
            await fetch('/api/admin/zoho/create-event', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
              },
              body: JSON.stringify(zohoEventData)
            });
          }
        } catch (zohoError) {
          console.log('Zoho Calendar integration not configured yet:', zohoError);
          // Continue with WhatsApp flow even if Zoho fails
        }

        toast({
          title: "¡Cita agendada!",
          description: result.message + " También se intentó agendar en Zoho Calendar.",
        });

        // Redirect to WhatsApp with pre-filled message
        const message = encodeURIComponent(
          `¡Hola! Quiero agendar una cita para mi vehículo:\n\n` +
        ...
        // Reset form
        setFormData({
          name: '', phone: '', email: '', serviceType: '', vehicleBrand: '', 
          vehicleModel: '', vehicleYear: '', urgency: '', preferredDate: '', 
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#004A9F] to-[#D71920] text-white p-6 rounded-t-2xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold font-helvetica">Agendar Cita</h2>
                <p className="text-white/90 font-helvetica">Reserva tu servicio automotriz</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-900 font-helvetica flex items-center">
              <Star className="w-5 h-5 mr-2 text-[#D71920]" />
              Tipo de Servicio
            </h3>
            <div>
              <label className="text-sm font-medium text-gray-700 mb-2 block font-helvetica">
          ...
          <div className="flex space-x-4 pt-6 border-t">
            ...
            <Button
              ...
              className="flex-1 bg-gradient-to-r from-[#004A9F] to-[#D71920] hover:from-[#003875] hover:to-[#b01319] text-white font-helvetica"
            >
              {isSubmitting ? (
                'Agendando...'
              ) : (
